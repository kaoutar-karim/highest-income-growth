...
class MysqlDriver():
    ...
    def create_db_connection(self):
        db_connection_str = f'mysql+pymysql://{const.USR}:{const.PWD}@localhost:3306/{self.database_name}'
        db_connection = create_engine(db_connection_str)
        return db_connection

    def read_table_from_db(self, table_name, index_col=None, list_columns:tuple = None):
        try:
            df = pd.read_sql(
                f"SELECT {','.join(list(list_columns)) if list_columns else '*'} FROM {table_name}",
                con=self.create_db_connection(), index_col=index_col)
        except:
            df = pd.DataFrame()
        return df

    def write_table_to_db(self, dataframe, table_name, if_exists='replace'):
        dataframe.to_sql(f'{table_name}', index= False, con=self.create_db_connection(), if_exists=if_exists)
